...
import { delayedDuplex, type Duplex, type LatencyOptions, tracedDuplex, Transport } from "effect-open-protocol"
import type { SimulatorNetwork } from "effect-open-protocol/simulator/SimulatorNetwork.ts"
...
export const traceDb = Flag.String("trace-db").pipe(
  Flag.withDescription("SQLite file every traced event is recorded into, for the trace UI"),
  Flag.withDefault(".effect-open-protocol/traces.sqlite")
)
