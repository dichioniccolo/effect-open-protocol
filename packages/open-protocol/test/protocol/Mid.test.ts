...
describe("Mid", () => {
  it("types each revision exactly", () => {
    ...
    expectTypeOf(SelectParameterSet.rev(1).reply).toEqualTypeOf<Mid.Accepted>()
  ...
  it.effect("round trips every revision through a frame", () =>
    Effect.gen(function* () {
      ...
      expect(yield* Mid.decode(second, dataOf(frame), deviceId)).toEqual(value)
    })
  )
  ...
  it.effect("decodes a class-backed revision into instances", () =>
    Effect.gen(function* () {
      ...
      const decoded = yield* Mid.decode(Job.rev(1), dataOf(frame), deviceId)
  ...
  it.effect("reports a body that does not decode", () =>
    Effect.gen(function* () {
      const exit = yield* Effect.exit(Mid.decode(ParameterSetSelected.rev(1), "0x7", deviceId))
