...
import { Effect, pipe, Ref, Scope, Stream, SubscriptionRef } from "effect"
...
const provided = <A, E, R>(effect: Effect.Effect<A, E, R>) =>
  Effect.provide(Effect.provide(Effect.scoped(effect), layerInMemory), layerNetwork)
...
describe("shutdown", () => {
  ...
  it.effect("stops delivering once the pool is closed", () =>
    provided(Effect.gen(function* () {
      ...
      const simulator = yield* makeSimulator({ endpoint })
      const scope = yield* Scope.make()
      const pool = yield* Scope.provide(makePool(), scope)
