...
Use a named `S.Literals([...])` schema when the literal set needs any of:

- guards (`S.is(Kind)`)
- the literal tuple (`Kind.literals`)
- subsets (`Kind.pick([...])`)
- member mapping (`Kind.mapMembers(...)`)
- direct annotation (`.annotate({ identifier, description })`)

Inline array literals passed directly to `S.Literals(...)` should omit
`as const`; `S.Literals` preserves them through const type parameters.
...
1. Define the literal domain with `S.Literals([...])`.
...
- duplicate literal arrays or enum-like constants for an `S.Literals` domain
...
1. Look for an existing schema in the project.
...
- Duplicate literal constants for reusable literal domains
  Replace with one named `S.Literals([...])` schema, `.literals`, and `.pick([...])`
- Ad-hoc string predicates
  Replace with local shared schemas, built-in checks, or a reusable metadata-rich
  filter
- Inline `S.Literals(...)` repeated across modules for one domain
  Replace with one exported, annotated `S.Literals(...)` schema
...
- Does the code reuse existing project schemas and built-in checks?
