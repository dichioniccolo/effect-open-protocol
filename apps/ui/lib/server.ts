 ...
 * been recorded. `EFFECT_OPEN_PROTOCOL_TRACE_DB` points it elsewhere; the default is the file
 * the CLIs write when run from the repo root, seen from `apps/ui/`.
 */
...
const traceDb = Config.String("EFFECT_OPEN_PROTOCOL_TRACE_DB").pipe(
  Config.withDefault("../../.effect-open-protocol/traces.sqlite")
)
