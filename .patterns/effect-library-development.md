...
### Error Handling with S.TaggedError

Create structured, typed errors using `TaggedError` from `effect/Schema`:
...
### Immediate Type-Check Pattern

**MANDATORY**: Type-check immediately after editing TypeScript files:
...
```bash
# 1. Check types (also catches broken code in touched modules)
bunx tsc --noEmit

# 2. Run the affected tests
bun run test test/ModifiedModule.test.ts

# 3. Run the full test suite before handing work back
bun run test
```
...
export class FeatureConfig extends S.Class<FeatureConfig>("FeatureConfig")(
  {
    option1: S.String,
    option2: S.Number
  },
