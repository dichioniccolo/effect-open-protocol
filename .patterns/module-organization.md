...
Established patterns for organizing modules in the Effect library, based on analysis of the core codebase structure and conventions. Paths below are relative to the Effect reference checkout at `.repos/effect` (set up with `bash scripts/setup-effect-ref.sh`); apply the same conventions to this repo's `src/`.
...
**Main index file pattern (.repos/effect/packages/effect/src/index.ts):**
...
**Internal organization (.repos/effect/packages/effect/src/internal/array.ts):**
