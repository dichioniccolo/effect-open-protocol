...
class MissingConfigError extends S.TaggedError<MissingConfigError>()(
  "MissingConfigError",
  { key: S.String },
...
- For `Option`-valued object fields, use `R.getSomes({...})` to drop `None` entries, `O.all({...})` when the whole object is all-or-nothing, and `S.OptionFrom*` when optionality/nullability belongs at the schema boundary.
...
class CreateTaskInputError extends S.TaggedError<CreateTaskInputError>()(
...
export class CreateTaskInput extends S.Class<CreateTaskInput>("CreateTaskInput")({
  id: S.String,
  title: S.String,
  priority: S.Int
}) {}
...
- Service identity is the string key passed to `Context.Service<Self, Shape>()("Key")`.
...
- Service keys must be unique across the application; namespace them (`"billing/InvoiceRepository"`) when names could collide.
...
- When behavior claims to hold across a schema-modeled domain, prefer
  `Arbitrary.schema(sourceSchema)` with FastCheck over happy-path fixture data
...
- Exported APIs in `src/` require JSDoc.
- Examples must compile.

### EF-12: Schema annotation is required

- New schemas must include meaningful annotation metadata: the `S.Class` annotations argument (`{ description }`) or `.annotate({ identifier, description })` / `S.annotate({ identifier, description })`.
...
- Model internal literal domains as one named `S.Literals([...])` schema; derive guards with `S.is(...)`, subsets with `.pick([...])`, and branch with `Match`.
- Do not add `as const` to inline array literals passed directly to `S.Literals(...)`; it uses const type parameters already.
...
- If schema properties are a union of literal strings (for example `kind`,
  ...
  `S.Literals([...])`, `.mapMembers`, and `Tuple.evolve`, then finalize with
...
const ExternalJobKind = S.Literals(["created", "completed"])

export class ExternalJobCreated extends S.Class<ExternalJobCreated>("ExternalJobCreated")(
  {
    kind: S.tag("created"),
    id: S.String
  },
...
export class ExternalJobCompleted extends S.Class<ExternalJobCompleted>("ExternalJobCompleted")(
  {
...
export const ExternalJobEvent = ExternalJobKind
  ...
  .annotate({ identifier: "ExternalJobEvent",
...
}).annotate({ identifier: "InternalJobEvent",
...
class UserJsonError extends S.TaggedError<UserJsonError>()(
  "UserJsonError",
  { message: S.String },
  {
...
export class CreateOrderInput extends S.Class<CreateOrderInput>("CreateOrderInput")(
  {
    orderId: S.String,
    customerId: S.String
  },
...
export class VersionSyncOptions extends S.Class<VersionSyncOptions>("VersionSyncOptions")(
  {
...
- Model internal literal domains as one named, annotated `S.Literals([...])`
  schema; use `.literals`, `.pick([...])`, and `S.is(...)` instead of parallel
  helpers.
- Use `S.Literals([...]).transform([...])` when a literal protocol/code map
  needs both encoded and decoded sides.
...
- Use `S.Literals` APIs (`.literals`, `.pick`, `.mapMembers`, `.transform`)
  plus `S.is(...)` instead of duplicate literal arrays, enum-like objects, and
  ad-hoc literal guards.
- Use class-local statics for repeated `decode`, `encode`, `arbitrary`, and
  ...
  import cycles.
- Reference: `.repos/effect/packages/effect/SCHEMA.md` (tagged unions and literals).
...
Pass an identifier (`S.TaggedError<DomainError>("billing/DomainError")(...)`)
only when a distinct namespaced schema identifier is wanted; otherwise omit it
as above. Never pass an identifier equal to the tag.
...
### Template: Match, S.Literals, or effect/Schema TaggedUnion over switch

```ts
import { Match, Tuple } from "effect"
import * as A from "effect/Array"
import * as S from "effect/Schema"

const Phase = S.Literals(["draft", "running", "done"])
type Phase = typeof Phase.Type

const TaggedPhase = Phase.mapMembers(Tuple.evolve([
  (draft) => S.Struct({ phase: draft }),
  (running) => S.Struct({ phase: running }),
  (done) => S.Struct({ phase: done })
])).pipe(S.toTaggedUnion("phase"))
type TaggedPhase = typeof TaggedPhase.Type

const phaseLabel = Match.type<Phase>().pipe(
...
const matchPhase = (taggedPhase: TaggedPhase): Phase =>
  TaggedPhase.match(taggedPhase, {
    draft: ({ phase }) => phase,
    running: ({ phase }) => phase,
    done: ({ phase }) => phase
  })
...
export class SomeErrorOne extends S.TaggedError<SomeErrorOne>()("SomeErrorOne", { message: S.String}) {}
export class SomeErrorTwo extends S.TaggedError<SomeErrorTwo>()("SomeErrorTwo", { message: S.String}) {}
...
10. New schemas are annotated with `identifier`/`description` metadata.
...
32. Finite variants, lifecycle states, status/result cases, and case-specific payloads are modeled as discriminated unions; literal-string discriminants and internal literal domains use a named `S.Literals([...])` schema with `.mapMembers`, `.pick`, and `S.is(...)`.
