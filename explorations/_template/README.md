# <Exploration Title>

## Status

<!-- Keep in sync with ops/manifest.json on every stage/status change. -->
Stage: `capture`
Status: `active`

Source: [`ops/manifest.json`](./ops/manifest.json)

## Spark

<One or two sentences: what itch started this exploration.>

## Next Open Question

<The single question the next session should pick up. Use `None — ready for
<next stage>` when the current stage is clean. Keep this current: it is the
cold-session resume point.>

## Read This First

1. [`ops/manifest.json`](./ops/manifest.json) - machine state: stage, status, open questions.
2. [`CAPTURE.md`](./CAPTURE.md) - raw dump (stage 0).
3. [`RESEARCH.md`](./RESEARCH.md) - prior art + capability inventory (stage 1, if present).
4. [`DECISIONS.md`](./DECISIONS.md) - grilling log (stage 2, if present).
5. [`BRIEF.md`](./BRIEF.md) - shaped pitch (stage 3, if present).
6. [`MAP.md`](./MAP.md) - decomposition (stage 4, if present).

## Trail

<Dated one-liners, newest first: what each session did and where it stopped.>

- YYYY-MM-DD: packet opened.
