...
- ~~Bug: opening a TCP connection has no timeout. Found on a real Rexroth Nexo
  ...
  timeout next to the other timeouts in `DeviceSettings`, with a test.~~ Fixed 2026-09-19. `connectTimeout` (10 s by default) bounds each attempt in `DeviceConnection`, whatever the transport.
...
- ~~Bug: a result recovered with MID 0064/0065 is acknowledged with MID 0062,
  ...
  `OldResult`, `GapRecovery`), with a test that asserts no 0062 after a 0065.~~ Fixed 2026-09-19. The ack travels with the pushed value (`ResultDelivery.submitPushed`), so a recovered result has none to send.
...
- ~~Bug: the reconnect backoff never resets after a healthy session, although
  ...
  the schedule once a session reaches `Ready`, with a `TestClock` test.~~ Fixed 2026-09-19. The reconnect schedule starts over at the first failure of a streak (`isStreakStart`). The `TestClock` test fails on the old code.
