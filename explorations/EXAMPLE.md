...
what changes on disk. The packet, the app it describes, and its module paths
are all hypothetical — they illustrate the process, not this repo's code.
...
> (manifest: `stage: capture`, `status: active`; README Status block
> filled). INBOX.md: bullet 1 removed, bullet 2 struck through with
> "→ canvas research".
...
> ▸ Agent searches `src/**` exports and barrels, scans relevant module
> docs/source, and fans out web searches.

**Agent:** RESEARCH.md written. The load-bearing findings:

- **In repo:** `src/drivers/uspto` already wraps the USPTO ODP API —
  the data source exists. `src/workspace/domain` has `EmailArtifact`
...
- **Constraint discovered:** the runtime doctrine already requires candidate
  writes + human approval (a hypothetical `goals/agentic-professional-runtime`), which is
...
| --- | --- | --- | --- |
| `deadline-extraction-lane` | USPTO email → evidence-backed CandidateTask | none | `src/drivers/uspto`, `src/workspace/domain` EmailArtifact + CandidateTask + ApprovalGate |
| `matter-timeline-surface` | timeline view of approved deadline tasks in professional-desktop | extraction lane | `src/ui`, atom patterns from `src/desktop` — plus NET-NEW: date parsing |

One challenge before you read further: I marked date parsing NET-NEW, then
checked the codebase — `effect/Schema` ships DateTime codecs
(`DateTimeUtcFromMillis`, `DateTimeUtcFromString`, …) and the nlp driver handles
...
> ▸ MAP.md finalized with the corrected citation. DECISIONS.md gains the epitaph:
...
cross-linked. README Status block: `graduated`, with both goal links.
